...
- ~~Two keypad-scan implementations coexist~~ — **fixed**: `keypad_checkpress` and `PrintKey` (both in `Keypad.c`, duplicating the row/column scan with different read primitives) are consolidated into a single `keypad_checkpress()` built on `DIO_Read`; the now-unused `DIO_u8read` was removed from `DIO.c`/`DIO.h`.
